...
#include "MMOAbilityTask_WaitTargetData.h"
...
UMMOAbilityTask_WaitTargetData::UMMOAbilityTask_WaitTargetData(const FObjectInitializer& ObjectInitializer)
...
UMMOAbilityTask_WaitTargetData* UMMOAbilityTask_WaitTargetData::MMOWaitTargetData(UGameplayAbility* OwningAbility, FName TaskInstanceName, TEnumAsByte<EGameplayTargetingConfirmation::Type> ConfirmationType, TSubclassOf<AGameplayAbilityTargetActor> InTargetClass)
{
  UMMOAbilityTask_WaitTargetData* MyObj = NewAbilityTask<UMMOAbilityTask_WaitTargetData>(OwningAbility, TaskInstanceName);		//Register for task list here, providing a given FName as a key
...
UMMOAbilityTask_WaitTargetData* UMMOAbilityTask_WaitTargetData::MMOWaitTargetDataUsingActor(UGameplayAbility* OwningAbility, FName TaskInstanceName, TEnumAsByte<EGameplayTargetingConfirmation::Type> ConfirmationType, AGameplayAbilityTargetActor* InTargetActor)
{
  UMMOAbilityTask_WaitTargetData* MyObj = NewAbilityTask<UMMOAbilityTask_WaitTargetData>(OwningAbility, TaskInstanceName);		//Register for task list here, providing a given FName as a key
...
void UMMOAbilityTask_WaitTargetData::Activate()
{
...
bool UMMOAbilityTask_WaitTargetData::BeginSpawningActor(UGameplayAbility* OwningAbility, TSubclassOf<AGameplayAbilityTargetActor> InTargetClass, AGameplayAbilityTargetActor*& SpawnedActor)
{
  SpawnedActor = nullptr;
...
void UMMOAbilityTask_WaitTargetData::FinishSpawningActor(UGameplayAbility* OwningAbility, AGameplayAbilityTargetActor* SpawnedActor)
{
...
bool UMMOAbilityTask_WaitTargetData::ShouldSpawnTargetActor() const
{
  check(TargetClass);
  check(Ability);
...
void UMMOAbilityTask_WaitTargetData::InitializeTargetActor(AGameplayAbilityTargetActor* SpawnedActor) const
{
  check(SpawnedActor);
  check(Ability);
  ...
  SpawnedActor->TargetDataReadyDelegate.AddUObject(this, &UMMOAbilityTask_WaitTargetData::OnTargetDataReadyCallback);
  SpawnedActor->CanceledDelegate.AddUObject(this, &UMMOAbilityTask_WaitTargetData::OnTargetDataCancelledCallback);
}

void UMMOAbilityTask_WaitTargetData::FinalizeTargetActor(AGameplayAbilityTargetActor* SpawnedActor) const
{
  check(SpawnedActor);
  check(Ability);
...
void UMMOAbilityTask_WaitTargetData::RegisterTargetDataCallbacks()
{
  ...
  if (!bIsLocallyControlled)
  {
    ...
    if (!bShouldProduceTargetDataOnServer)
    {
      ...
      AbilitySystemComponent->AbilityTargetDataSetDelegate(SpecHandle, ActivationPredictionKey).AddUObject(this, &UMMOAbilityTask_WaitTargetData::OnTargetDataReplicatedCallback);
      AbilitySystemComponent->AbilityTargetDataCancelledDelegate(SpecHandle, ActivationPredictionKey).AddUObject(this, &UMMOAbilityTask_WaitTargetData::OnTargetDataReplicatedCancelledCallback);
...
void UMMOAbilityTask_WaitTargetData::OnTargetDataReplicatedCallback(const FGameplayAbilityTargetDataHandle& Data, FGameplayTag ActivationTag)
{
...
void UMMOAbilityTask_WaitTargetData::OnTargetDataReplicatedCancelledCallback()
{
  check(AbilitySystemComponent);
...
void UMMOAbilityTask_WaitTargetData::OnTargetDataReadyCallback(const FGameplayAbilityTargetDataHandle& Data)
{
  check(AbilitySystemComponent);
  if (!Ability)
  {
...
void UMMOAbilityTask_WaitTargetData::OnTargetDataCancelledCallback(const FGameplayAbilityTargetDataHandle& Data)
{
  check(AbilitySystemComponent);
...
void UMMOAbilityTask_WaitTargetData::ExternalConfirm(bool bEndTask)
{
  check(AbilitySystemComponent);
  if (TargetActor)
  {
...
/** Called when the ability is asked to confirm from an outside node. What this means depends on the individual task. By default, this does nothing other than ending if bEndTask is true. */
void UMMOAbilityTask_WaitTargetData::ExternalCancel()
{
  check(AbilitySystemComponent);
  Cancelled.Broadcast(FGameplayAbilityTargetDataHandle());
...
void UMMOAbilityTask_WaitTargetData::OnDestroy(bool AbilityEnded)
{
  if (TargetActor)
  {
    TargetActor->Destroy();
...
bool UMMOAbilityTask_WaitTargetData::ShouldReplicateDataToServer() const
{
